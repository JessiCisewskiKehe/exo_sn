...
\noindent RESPONSE:  We would like to begin by thanking the reviewer for the helpful comments both in this review and the original manuscript.  We are grateful for these helpful suggestions, and believe they have dramatically improved this work along with giving us an opportunity to clarify ambiguities.  Below we address the new comments individually.  Updated statements in the main text are presented in bold, and statements that are no longer relevant have been deleted.
...
After redoing the SOAP 2.0 simulations and injecting some noise corresponding to a S/N of 100, it seems that both the spot and facula induce a negative correlation between the CCF asymmetry and its barycenter (i.e. the different RV estimators). Therefore we cannot have anymore a cancelation effect. Those negative correlations does not match the positive correlations always observed in the 5 stars studied in the paper. This implies that SOAP 2.0 is not able to model properly the CCF asymmetry variations. This could be due to the fact that SOAP 2.0 uses the spectrum of a spot to model the magnetic field effect inside a facula. However, we know that the temperature of a facula is significantly different that the one of a spot. We discussed this discrepancy in the text.
...
%
\item[]  RESPONSE:  We considered the noise on the different parameters of the Normal and SN fit to the CCF for a SNR of 100, which is typical for high-precision RV measurements. 

%We updated the entire Section 4 of the manuscript with the newly obtained results. 
%
...
%
\item[]  RESPONSE:  For all the stars, we observe a similar positive gamma -- SN RV correlation.
The signal on Alpha Centauri B is likely due to a facule (Dumusque 2014) so this would make sense.
But we also see a positive correlation for Corot-7. This star is extremely active, rotates fast, and the activity signal
should be mainly driven by spots. Therefore we are not sure that we can use this gamma -- SN RV correlation to differentiate between
spots and faculae.
\bigskip
%
%
