...
\title{Cross correlation function line-profile variations in radial-velocity measurements by fitting a Skew Normal distribution
...
{{\bf An exoplanet or stellar companion will induce a variation in the radial velocity (RV) of its host star, which will translate into a pure-Doppler shift of the stellar spectrum and thus a velocity shift of the cross-correlation function (CCF) without changing its shape. The barycenter of the CCF is therefore a measure of the stellar RV. We generally measure this barycenter by fitting a Normal distribution to the CCF and then retreving its first moment, i.e. its mean. However, some astrophysical processes, like oscillations, granulation, stellar activity and single-lined spectroscopic binaries will modify the asymetry of the CCF, which will introduce a spurious radial-velocity variation and perturb the detection of exoplanets. To decorrelate the signals introduced by exoplanets from the one introduced by other astrophysical processes, we use the higher moments of the CCF. Note that in the case of high-precision RV surveys, mainly focusing on non-blended GKM dwarfs, stellar activity is the main perturbing signal.}}
{We propose here to measure the moment of the CCF using a Skew Normal (SN) distribution, which compared to the Normal distribution generally used, provides an extra parameter to model the CCF natural asymmetry.}
...
By measuring the radial-velocity of a star, it is possible to detect the stellar wobble induced by the gravitational pull of orbiting planets. This so-called radial-velocity (RV) technique is an indirect method as the planet cannot be seen. This poses some difficulties as the RV technique is not only sensitive to planetary signals, but also to different stellar signals. At the meter-per-second level, RV measurements are affected by oscillations \citep[][]{Arentoft-2008,Kjeldsen-1995,Mayor-2003}, granulation \citep[][]{Meunier-2016,Dumusque-2011a,Lindegren-2003,Del-Moro-2004} and activity signals \citep[][]{Dumusque-2014,Meunier-2011,Saar-2009,Desort-2007,Saar-1997}. Other effects can also introduce RV variations, like single-lined spectroscopic binaries (SB1). High-precision RV surveys tends to avoid targeting any SB1 and to observe GKM dwarfs to minimize the impact of oscillations and granulation. High-precision RV surveys also tend to observe quiet stars in terms of stellar activity, however, as most of the star present solar-like magnetic cycles \citep[][]{Lovis-2011b}, it is not possible to observe the same star at a quiet stage for several continous years.
Stellar activity is therefore the main limitation to the detection of Earth-like planey





While the gravitational pull of an orbiting planets introduces a pure-Doppler

The radial velocity (RV) technique is an indirect method to detect exoplanets. 


By measuring the radial velocity induced by the stellar wobble induced by a planet orbiting its host star
...
\end{eqnarray}
%
The CP allows us to use the usual methods offered by the inferential theory and we will use this parametrisation for the rest of the paper. Further details about DP, CP and statistical properties of the SN are treated in rigorous mathematical and statistical viewpoints in the book by \cite{Azzalini2014}.
%-----------------------------------------------------------------------------------------------------------------------------------------------
...
In the case of the errors when using the SN fitting, the precision on the SN RVs is however 60\% worse than for the RVs. This comes from the fact that because the asymmetry is accounted for in the SN and because the SN RVs and $\gamma_1$ are strongly correlated, the errors in $\gamma_1$ propagates to the errors in SN RVs. This is not so important for SNR $>$ 150 for which the errors are below the \ms precision of HARPS and the \ms variation induced by stellar activity signals even in low activity cases \footnote{As we can see in papers publishing HARPS results using an additional jitter to consider stellar activity noise not accounted for in the \emph{noise} parameter, the extra jitter is always greater than 0.8\ms \citep[e.g.][]{Udry:2017aa,Feng:2017aa,Diaz:2016ab}.}. These larger errors in RVs can be a problem for low SNR measurements. In the case of the CCF width, the error derived when using the Normal or SN fitting are very similar. Finally for the asymetry, we see that the error in $\gamma_1$ is $\sim$15\% smaller than the error in BIS SPAN.
