...
\includegraphics[height = 6in]{FACULAE_NEW_CORRECTION_[3]CorrectionActivity_RadialVelocity_vs_time.pdf} 
   ...
   %
 (bottom) The residuals from the model fit using Eq.~\ref{eq:RV:correction} (new correction) and the usual correction \jessi{it might be helpful to actually have an equation for the usual/old model}.  The residuals have a smaller systematic component when using the proposed model of 
  Eq.~\ref{eq:RV:correction} (blue circles) compared to the usual model (blue triangles).
The estimated parameters are presented in Table \ref{table:faculae.test} \jessi{This table only has p-values and $R^2$}. }
