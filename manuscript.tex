...
\includegraphics[height = 6in]{NEW_CORRECTIONHD12862_[3]CorrectionActivity_RadialVelocity_vs_time.pdf} 
 ...
 (bottom) The residuals from the model fit using Eq.~\ref{eq:RV:correction} (New corr. std, black dots) and the residuals from the usual correction (Usual corr. std, blue triangles), based on $RV_{\text{activity}}=\beta_0+\beta_1 \gamma + \beta_2 \text{SN FWHM}$ for the SN fit and on $RV_{\text{activity}}=\beta_0+\beta_1 \text{BIS SPAN} + \beta_2 \text{FWHM}$ for the normal fit. The residuals have a smaller systematic component when using the proposed model of 
  Eq.~\ref{eq:RV:correction} (black dots) compared to the usual model (blue triangles). \jessi{The bottom plot has black triangles rather than black dots...}}
...
\includegraphics[height = 6in]{[5]Errors_vs_SNR_all_stars.pdf} 
   \caption{\jessi{The stars used should be noted here}  Comparison between the standard errors from the bootstrap analysis of the estimated RVs, FWHM, and asymmetry parameters. When using SN mean RV (black circles), the standard errors are in average $60\%$ larger than the standard errors of N mean RV (red triangles). However, the standard errors for SN median RV (cyan crosses) are on average $10\%$ smaller than the standard errors coming from the N mean RV. 
...
Indeed, in the case were photon-noise is not dominant, stellar activity can be better characterized if its effect can be amplified by measuring the SN mean RV, even if the white noise in the data is increased by 60\%. \jessi{white noise of the estimator?  The previous two sentences are not very convincing to me.}

Even if the different RV estimators generally have different amplitudes, once the new correction for stellar activity is applied, the residuals of the model have similar rms.  This suggests that for an activity correction based on a linear combination of CCF parameters, the SN density fit does not help. \jessi{The previous sentence does not seem to be a good sentence...I'm not sure what the intent of it is so I'm not sure how to fix it.}
When comparing the activity correction proposed in this paper with the usual correction that only uses a linear combination of the CCF asymmetry and width, there appears to be only a slight improvement. \jessi{In the previous sentence, are you referring to the real data only?  The SOAP data did show improvement - right?}
We however encourage the community to use this new simple linear model if the goal is to reduce activity without considering the computationally expensive use of Gaussian Process. \jessi{The previous sentence makes no sense given the discussion in this paragraph.  You are saying the proposed correction doesn't help, and then you say to use it...}
...
In this Appendix, a similar analysis as presented in Sec.~\ref{sec:4} is discussed for four main-sequence stars: HD192310 (K2V), HD10700 (G8V), HD215152 (K3V) and finally Corot-7 (K0V). \jessi{Are there papers that can/should be cited here for the datasets used?}
...
The correlation between $\gamma$ and SN mean RV is stronger than the correlation between the BIS SPAN and Normal mean RV or between the asymmetry parameters derived in \citet{Boisse-2009} and \citet{Figueira-2013} and RV \jessi{was the RV for these other asymmetry papers the Normal mean RV?} for all the considered stars. 
The correlation between SN FWHM and SN mean RV is stronger than the correlation between FWHM and RV \jessi{N mean RV?} for three of the four stars.  Also for all these stars the originally estimated RVs were corrected from spurious variations in RVs caused by stellar activity using Eq. \ref{eq:RV:correction}. 
...
However, the rms of the residuals for $\text{HD}215152$ are 0.05\,\ms lower for the SN model than the Normal model, and the rms of the residuals for Corot-7 are 0.34\,\ms lower.  $\text{HD}215152$ and Corot-7 have  lower SNR than the other two analyzed stars.  \jessi{was the SNR of the four stars stated anywhere?  If not, if we want to keep the last point, we should note what the SNR of the stars are.}
...
The correlations between the asymmetry parameters and their corresponding RVs are stronger when using the SN mean RV compared to N mean RV. 
The correlation between the FWHM and the N mean RV, only for this quiet star, is stronger the the analogous correlations with the estimated SN RVs. The estimated $R$ are statistically significant, except for the correlation between FIG BIS and RV (p--values=0.36).}   
...
The p--values associated with each $R$ are, from the top to the bottom, from the left to the right: $2.2e-16,0.02,0.27, 0.05 ,7e-4,1e-5,1e-5,0.0007,0.0004,0.5,0.2$. \jessi{It is probably best to just note the ones that do not have statistically significant p-values...}}
...
The correlations are consistently stronger when using SN mean RV compared to N mean RV.
The p--values associated with each $R$ are, from the top to the bottom, from the left to the right: $2.2e-16,2e-5,0.23,0.11,0.0055,0.0025,0.012,0.002,2.2e-16,2.2e-16,2.2e-16$. \jessi{maybe just note the ones that are not statistically significant.}}
