...
\xavier{It is clear in the case of Alpha Cen B, that the correlation found between $\gamma$ and SN mean RV is the strongest. 
...
When looking at the correlation between the width and the RV estimate for Alpha Cen B, once again the correlation is the strongest for the SN parameters, with $R=0.82$ for SN FWHM-SN mean RV, while it is equals to $R=0.7$ for the correlation between FWHM and N mean RV.}
%
...
\xavier{Although, when studying the correlations between the different parameters, we see that the parameters derived using the SN density are more sensitive to stellar activity than with the Normal density,
the proposed linear model for correcting stellar activity is not able to perform better in the SN case than in the Normal case.}
...
\xavier{Comparing the new correction proposed in Sec.~\ref{sec:31} to correct for activity and the usual correction only using a linear combination of the width and the asymmetry of the CCF, we see very similar efficiency. The new correction is however able to get RV rms that are 6\,\cms\,smaller than the usual correction.}
...
\xavier{The preceding section was analysing the improvement brought by the SN parameters compared to the Nomral parameters and the BIS SPAN. Here we do a similar analysis for four other main sequence stars: HD192310, HD10700, HD215152 and Corot-7. The same correlation plots, and activity-correction plots that was found in the preceding section can be found found for those new stars in Appendix \ref{appendix}.

When looking to the correlation between the parameters, we arrive to the same conclusions than for Alpha Cen B. The correlation between $\gamma$ and SN mean RV is always the strongest among all the other asymmetry-RV corrleations not derived with SN parameters. For the correlation between the width and the RV, the strongest correlation is also found for all the stars between SN FWHM and SN mean RV. There is one exception however, in the case of HD10700, the Pearson correlation coefficient between FWHM and N mean RV is equal to $R=0.53$, while it is only $R=0.42$ for the correlation between SN FWHM and SN mean RV.
...
In terms of activity correction, we see like in the case of Alpha Cen B, that although the RV rms is larger when measuring the RVs using SN mean RV compared to the RVs derived using N mean RV, after correcting for activity using the new model proposed in Sec.~\ref{sec:31}, both RV estimates gives similar residuals. We also see nearly no significant differences between the new correction proposed and the usual correction only using a linear combination of the width and the asymmetry of the CCF. There is only in the case of Corot-7 where the new correction is able to get a RV residual rms 30\,\cms smaller than the usual correction.}
