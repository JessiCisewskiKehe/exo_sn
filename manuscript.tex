...
%Therefore, fitting the CCF using a model that naturally includes an asymmetry, like the SN distribution, should give in principle more precise results.
...
\end{eqnarray}
%
where C is an unknown offset for the continuum of the CCF, A is the unknown contrast of the CCF \jessi{what is meant by the ``contrast''?} and $\mu$, $\sigma^2$ and $\gamma$ are the mean, variance and skewness of the SN as defined above.
The values $y_1, \ldots, y_n$ are the different values of the abscisse \jessi{what is this previous word?} of the CCF, generally with the dimension of a velocity. \jessi{what is meant by ``with the dimension of a velocity''?  Do you mean units?}

%%%Since the CCF has an asymmetry due the convective blueshift, the SN density should in principle better catch this aspect, together with other changes in asymmetry, with respect to fitting a Normal distribution. 
%%%To initially check this intuition, we compared the CCF residuals after fitting a Normal and a SN distribution for 2 stars. The first star is Alpha Centauri b, whose CCF's have high signal-to-noise ratio (SNR). The second star is Corot-7, whose CCF's have low SNR. Fig.~\ref{fig:Residual.comparison} shows that the SN seems to be a slightly better model to explain the shape of the CCF.%, in particular as the SNR decreases.
%%%%
%%%\begin{figure*}[htbp]
%%%   \centering
...
When fitting a Normal distribution to the CCF, the estimated mean of the model is used as the estimated RV, the FWHM of the Normal density ($2\sqrt(2\ln2)\sigma$ with standard deviation $\sigma$) represents the width of the CCF.
...
With the proposed SN approach, we propose two estimators of the RV: the mean and median of the SN model fit (referred to as \emph{SN mean RV} and \emph{SN median RV}, respecitvely) , and present advantages and limitations for both of these choices in Sec. \ref{sec:4} and Sec. \ref{sec:5}. 
The width of the SN, \emph{SN FWHM}, is defined in the same way as for the Normal distribution\footnote{Note that SN FWHM does not correspond to the width of the SN distribution at half maximum like in the Normal case.}, and finally the skewness of the CCF, \emph{SN GAMMA}, is estimated by the $\gamma$ parameter.
