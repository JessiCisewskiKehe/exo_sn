...
For each period considered, the minimum amplitude at which at least 80\% of the planets with different phases were detected was found. 
The results for the proposed and usual correction are shown in Fig.~\ref{fig:detection_limits}. 
We can see that the new correction for stellar activity based on Eq.~\eqref{eq:RV:correction} slightly improves the detection limit of the exoplanet compared to the usual approach, and the three estimators of RV give similar detection limits.
These results suggest that any of the RV estimators can be used when searching for a planetary signal in RV data contaminated by stellar activity, though additional investigation is needed.
...
\includegraphics[height = 2.6in]{detection_limits.pdf} 
 ...
 \jessi{to be consistent with other figures, "NEW" and "USUAL" should only have the first letter uppercase.}
   }
