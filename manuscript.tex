...
\jessi{The SN Mean and Median bottom plots still show the two outliers - in the updated plots emailed out by Umberto it had looked like the outliers were at least somewhat resolved.  Also, it would be good to clarify what the std is in the top plots (e.g. Estimated RV std) since there are three sets of points.}
}
...
This behavior is similar to the faculae (see Sec.~\ref{sec:soap.faculae}) suggesting that the activity signal could be due to faculae present on Alpha Centauri B; \jessi{the previous statement seems like rather weak evidence - is there more that could be said to strengthen this point?  Or are there plots that could be referenced/compared?}
...
Similar to the anayses in Sec.~\ref{sec:soap}, we compare the correlation between the asymmetry or the width parameters of the CCF and the RV in Fig.~\ref{fig:alphacent:corrPlot}. 
...
It is clear in the case of Alpha Cen B, but also in the four other stars presented in Appendix \ref{appendix}, that the correlation found between $\gamma$ and SN mean RV is consistently the strongest. 
In particular, for Alpha Cen B, the Pearson correlation coefficient is $R=0.74$, while it is at best $R=-0.42$ for all the other asymmetry--RV correlations not derived using the SN density fit.

\jessi{The next paragraph is somewhat confusing because the other stars are now wrapped into the discussion without proper introduction.}
The correlation between SN FWHM and SN mean RV is the strongest for all the stars, except for the quietest star of all five, HD10700, for which the Pearson correlation coefficient is 0.53 for the Normal parameters and 0.42 for the SN parameters. This analysis shows that the parameters derived when using a SN density are more sensitive the stellar activity, and therefore using those parameters can lead to the detection of stellar activity, when the Normal parameters do not detect anything. This is the case for the asymmetry-RV correlation of HD10700, HD215152, Corot-7, and the width-RV correlation for HD215152 (see Appendix \ref{appendix}).
