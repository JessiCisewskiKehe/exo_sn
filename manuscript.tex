...
\xavier{Fig.~\ref{fig:faculae} shows the barycentric variation of the CCF induced by a simulated equatorial faculae of size 3\% relative to the visible stellar hemisphere. We compare the barycentric variation as measured when fitting a Normal density and taking its mean (RV), and when fitting a SN density and taking its mean (SN mean RV) or its median (SN median RV). Although the different estimates for the RV present similar variation as a function of rotational phase, we note that SN mean RV present a smaller amplitude than the two others.}
...
\xavier{In Fig.~\ref{fig:faculae.corr}, we compare the correlation between the different RV estimates and the different asymmetry or width estimates. As we can see, the strength of the correlation between $\gamma$ and SN mean RV is extremely similar to the correlation between BIS SPAN and the RV, with a Pearson correlation coefficient} \xavier{$R=-0.75$}. \xavier{Moreover, the correlation between $\gamma$ and SN median RV is much stronger, reaching} \xavier{$R=-0.95$}. \xavier{When comparing the correlation between the width and the different RV estimates, we observe that the data draw a circle, which implies a phase shift between those two parameters. In this case we find no correlation between those parameters, however, the different estimates gives very similar results.}
