...
\jessi{The SN Mean and Median bottom plots still show the two outliers - in the updated plots emailed out by Umberto it had looked like the outliers were at least somewhat resolved}
}
...
The proposed model from Eqn~\eqref{eq:RV:correction.overall} was fit and the results of the estimated model are summarized summarized in Table \ref{table:spotplanet.test}.   Except for the width parameters with coefficient $\beta_3$, all the other Normal or SN parameters are significantly useful to explain the RV variation induce by a spot plus a planet. 
It is also apparent that the residuals, once corrected for stellar activity and the presence of the planet, lead to similar results with SN mean RV giving the smallest value. \jessi{I'm not sure what the point of the previous sentence is - the SN mean RV residuals is similar to what?}
