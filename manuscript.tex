...
as we saw in Sec.~\ref{sec:soap.spot} that a spot induces a negative correlation between $\gamma$ and SN mean RV and weak correlations are measured between SN FWHM and SN mean RV.
...
\umberto{For each of the 4500 simulations considered, Eq. \ref{eq:RV:correction.overall} was applied. The use of Eq. \ref{eq:RV:correction.overall} leads to some additional comments. First of all we note that both the function that corrects for stellar activity and the keplerian signal must be simultaneously considered, as shown in Sec. \ref{sec:soap.spot.planet}. Because of the non linearity of the model that defines the orbital parameters of the exoplanets, non linear least squares \citep[][]{levenberg1944method,marquardt1963algorithm,teunissen1990nonlinear} must be used. In order to run the algorithm, a set of initial values for the parameters of interest has to be provided. Concerning the parameters related to the activity correction, we have chosen as initial values the ones obtained by only considering Eq.~\ref{eq:RV:correction}. The selection of the initial values for the orbital parameters of the exoplanet is more challenging. In the present study we assumed the circularity of the orbit, hence the parameters to estimate are the amplitude $K$, the epoch at the periapsis $t_0$ and the orbital period $P$. As initial value for $K$ we selected the maximum measured RV. The epoch at the periapsis $t_0$ is defined as that epoch such that the distance between the planet and the star is the smallest. Since in our simulation study the orbit of the planet in circular, the distance star--planet does not change over time, making the choice for $t_0$ arbitrary. We selected as initial value for $t_0$ that epoch for which the largest RV was measured. When providing an initial value for the orbital period $P$, we note that an initial value far from the true one can result in the non linear least squares algorithm to get stuck in local modes  \citep[][]{levenberg1944method,marquardt1963algorithm}, providing a wrong set of estimates for all the parameters of Eq. \ref{eq:RV:correction.overall} We note that in  Sec. \ref{sec:soap.spot.planet}, because the amplitude was equal to $K=10 \ms$, a good initial guess for the orbital period was available and local modes were avoided. For this reason we selected as initial guess for $P$ a value close to the true orbital period. This simplification is motivated by the fact that we are mainly interested in evaluating the goodness of the activity correction function presented in Eq.~\ref{eq:RV:correction} respect to the usual one, based on $RV_{\text{activity}}=\beta_0+\beta_1 \gamma + \beta_2 \text{SN FWHM}$ for the SN fit and on $RV_{\text{activity}}=\beta_0+\beta_1 \text{BIS SPAN} + \beta_2 \text{FWHM}$ for the normal fit. In order to properly do that, we need to correctly estimating the orbital parameters of the exoplanets as well. Because of the challenges related to the non linearity of the keplerian signal, we assumed to have a good initial guess for the orbital period. However, how to correctly estimating the orbital parameters of the extrasolar planet, in particular when no assumption on the eccentricity of the orbit are available, is something to investigate in future analyses.

Once the parameters involved in Eq. \ref{eq:RV:correction.overall} have been estimated,  signals in the residuals, defined as $RV - RV_{\text{activity}}$, were investigated using a Generalized Lomb-Scargle periodogram \citep[][]{Lomb-1976a, Scargle-1982, Zechmeister-2009}. If a signal with a P-value\footnote{The P-values were estimated using a bootstrap procedure.} smaller than 1\% had a period compatible with the injected planetary period within an error budget of 20\%, the signal was considered significant and the corresponding planet considered detected.  Finally, for each period considered, the minimum amplitude at which at least 80\% of the planets with different phases were detected was found. The results are shown in Fig.~\ref{fig:detection_limits}, where we also considered the usual function that corrects for stellar activity. We can see that the new correction for stellar activity based on Eq.~\ref{eq:RV:correction} slightly improves the detection limit of the exoplanet. Concerning the type of location parameter that defines the RV, all three of the different RV estimators give similar detection limits. Though more investigations should be considered, these results suggest that any of the RV estimators can be used when searching for a planetary signal in RV data contaminated by stellar activity.

\begin{figure}[!h]
\begin{center}
\includegraphics[height = 2.6in]{detection_limits.pdf} 
   \caption{Detection limits of planetary signals once the stellar activity signal is removed from the raw RVs using the model proposed in Eq.~\ref{eq:RV:correction} (solid lines) or the usual correction based on $RV_{\text{activity}}=\beta_0+\beta_1 \gamma + \beta_2 \text{SN FWHM}$ for the SN fit and on $RV_{\text{activity}}=\beta_0+\beta_1 \text{BIS SPAN} + \beta_2 \text{FWHM}$ for the normal fit (dashed lines). The correction for stellar activity based on Eq.~\ref{eq:RV:correction} slightly improves the detection limit and the different RV estimators have similar detection limits.}
...
This is likely caused by the fact that the periods of the simulated planets at 25 days are close to the first harmonic or to the rotational period of the star \citep[36.7 days,][]{Dumusque-2012} and therefore close to the semi-periodicity of the stellar activity signal.}
...
\umberto{We also proposed a new model to correct the estimated RV data for stellar activity signals, by using the amplitude of the CCF and an interaction term between the estimated asymmetry and the width parameters. Using simulated data from SOAP 2.0, this new proposed correction reduces the effect of the stellar activity signal by factors of about 2 and 3.5 over the usual model respectively for the facula and spot. When using real data and when injecting a fake planets, the detection limit slightly improved if using the new correction for stellar activity over the usual one.}
