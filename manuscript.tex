...
The proposed model from Eq.~\ref{eq:RV:correction.overall} was fitted to the RV data and the results of the estimated model are summarized in Table \ref{table:spotplanet.test}.   Except for the width parameters with coefficient $\beta_3$, all the other Normal or SN parameters are significantly useful to explain the RV variation induced by a spot plus a planet. 
\xavier{We also observe that the RV residuals, once corrected for stellar activity and the presence of the planet, are comparable in terms of rms for all the three different RV estimates, with SN mean RV giving a slightly smaller value.}
%\begin{figure*}[htbp]
%   \centering
...
This behavior is similar to the faculae simulated with SOAP 2.0 in see Sec.~\ref{sec:soap.faculae}, suggesting that the activity signal could be due to faculae present on Alpha Centauri B. \xavier{Another argument in favour of the faculae, is the presence of the positive correlation between $\gamma$ and SN mean RV and SN FWHM and SN mean RV seen in Fig.~\ref{fig:alphacent:corrPlot}. In Sec.~\ref{sec:soap.spot}, we saw that a spot induces a negative correlation between $\gamma$ and SN mean RV and no correlation between SN FWHM and SN mean RV}.
