...
In this case, sensitivity was measured using the correlation between the asymmetry parameter and the estimated RVs; {\bf{in the spot case SN median RV had a correlation with $\gamma$ as strong as the correlation between N mean RV and BIS SPAN (R= -0.81).}}
Moreover,  {\bf{when considering the case of a facula on the photosphere of the star}}, the correlation between the FWHM and the estimated RVs is stronger when using the parameters from the SN compared to the parameters from the Normal. {\bf{For the other presented cases, no statistically significant difference on the correlation between FWHM and the estimated RVs is obtained.}}
