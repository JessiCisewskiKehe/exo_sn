...
{\bf{ We consider for our simulations SNR=100.}}
...
%
{\bf{There is a stronger correlation between SN FWHM and SN mean RV ($R=0.92$) than between SN FWHM and SN median RV ($R=0.65$), but the correlation between FWHM and N mean RV is the weakest ($R=0.47$).}} 
...
{\bf{In addition the rms for N mean RV reduces from 8.02 \ms before correcting for stellar activity to about 1.2 \ms after once corrected for stellar activity using the proposed linear model; the respective values for SN median RV are 7.07 \ms to 0.88 \ms and for SN mean RV are 5.9 \ms to 0.88 \ms.}}
When comparing the correction proposed in Eq.~\eqref{eq:RV:correction} with what is generally used (i.e. a linear combination of only the asymmetry and width parameters), we see that the proposed correction is able to reduce the rms of the RV residuals by  {\bf{15 \%}}. Looking at the significance of the coefficients in Table~\ref{table:faculae.test}, we observe that {\bf{all the parameters corresponding to the SN variables are statistically significant. When using the Normal variables, the parameters $\beta_{3}$ and $\beta_{4}$ are not statistically helpful for the correction.}} 
...
{\bf{In addition the rms for N mean RV reduces from 6.14 \ms before correcting for stellar activity to about 3.04 \ms after once corrected for stellar activity using the proposed linear model; the respective values for SN median RV are 5.85 \ms to 2.74 \ms and for SN mean RV are 5.27 \ms to 2.74 \ms.}}
When comparing the activity correction proposed in this paper with what is commonly used, i.e only a linear combination of the width and asymmetry of the CCF, we see that our solution is capable of reducing the RV residual rms by {\bf{6 \%}}.
The Normal or SN parameters involved in Eq.~\eqref{eq:RV:correction} are all statistically significant to explain the activity signal as seen in Table~\ref{table:spot.test}, {\bf{except the width of the CCF and the variable that evaluates the interaction between the width and the asymmetry of the CCF.}}
