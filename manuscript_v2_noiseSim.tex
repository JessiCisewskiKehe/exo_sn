...
{\bf Another feature that suggests the presence of faculae is the statistically significant p--value associated to the interaction term $\gamma \cdot \text{SN FWHM}$ used in the proposed function to correct from stellar activity, as summarized in Table~\ref{table:alphacent.test}. In Sec.~\ref{sec:soap.faculae}, we saw that the interaction term was only significant in the case of faculae.
as we saw in Sec.~\ref{sec:soap.faculae} a facula produces as well a statistically significant p--value associated to the  interaction term $\gamma \cdot \text{SN FWHM}$}
